...
accidents = accidents.drop(columns=["index"])

accidentsall = accidentsall.reset_index()
accidentsall = accidentsall.drop(columns=["index"])

violations = violations.reset_index()
violations = violations.drop(columns=["index"])

violations["YEAR"] = violations["Violation Year"].astype(int)

#%% Trend Lines of Fatalities & Injuries in NYC
...
#%% Trend Lines of Fatalities & Injuries for each Borough
...
#%% Trend Lines of Traffic Violations in NYC Boroughs
...
#%% Top Known Reasons for Traffic Accidents
...
sns.set_theme(style="white")
hmd = topdeathfactors.pivot("PRIMARY CONTRIBUTING FACTOR", "YEAR", "Count")
...
for y in year:
    ...
    x = x.groupby([group,'YEAR'])["index"].count().reset_index().sort_values(by=["index"], ascending=False).iloc[0:5]
